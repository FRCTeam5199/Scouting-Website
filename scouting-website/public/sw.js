...
// Vite PWA / Workbox handles all asset caching automatically
...
async function syncOfflineData() {
  ...
  try {
    ...
    const request = indexedDB.open("scouting-db", 3);
    ...
    request.onsuccess = function (event) {
      ...
      getAllRequest.onsuccess = function () {
        ...
        console.log(`Background sync: ${allData.length} submissions`);
        ...
        allData.forEach(async (item) => {
          ...
          try {
            ...
            if (response.ok) {
              ...
              const deleteStore =
                deleteTransaction.objectStore("offline-data");

              deleteStore.delete(item.submissionId);

              console.log(`Synced submission: ${item.submissionId}`);
            ...
            console.log(
              `Sync failed for submission: ${item.submissionId}`
            );
