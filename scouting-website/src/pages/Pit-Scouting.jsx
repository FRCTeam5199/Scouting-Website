...
function PitScouting() {
  ...
  // Upload image to S3 via API Gateway + Lambda
  async function uploadImage(event) {
    ...
    reader.onload = async () => {
      ...
      await fetch(
        "https://abc123.execute-api.us-west-2.amazonaws.com/upload",
        {
          method: "POST",
          headers: {
  ...
  const onSubmit = async (values) => {
    ...
    try {
      if (isOnline) {
        ...
        setSuccessMessage("Form saved and sent to server!");
      } else {
        await saveOffline(submission);
        setSuccessMessage("Form saved locally. Will sync when online.");
      }
      setShowSuccess(true);
